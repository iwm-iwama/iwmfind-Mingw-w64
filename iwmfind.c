	...
	for (UINT _u1 = 0; _u1 < $ARGC; _u1++)
	{
		...
		if ((wp1 = iCLI_getOptValue(_u1, L"-i=", L"-in=")))
		{
			...
			else if (AryInDirSize)
			{
				P2(
					IESC_FALSE1
					"[Err] Dir と -in は併用できない!" IESC_RESET);
				imain_end();
			}
			else
			{
	...
	if (*_Md || *_Cp || *_Mv || *_Mv2 || *_Ext || *_Ext2 || _Trashbox || *_Rep || _Rm)
	{
		...
		else if (*_Rep)
		{
			if (!iF_chkExistPath(_Rep))
			{
				...
				P(
					IESC_FALSE1
					"[Err] --replace '%s' は存在しない!" IESC_RESET
					"\n",
					mp1);
				ifree(mp1);
				imain_end();
...
VOID print_help()
{
	P1(
		...
		"\n"
		"\033[9G"
		"ctime_cjd : 作成日時     -4712/01/01 00:00:00始点の通算日／CJD=JD-0.5"
		"\n"
		"\033[9G"
		"ctime     : 作成日時     \"yyyy-mm-dd hh:nn:ss\""
		"\n"
		"\033[9G"
		"mtime_cjd : 更新日時     ctime_cjd参照"
		"\n"
		"\033[9G"
		"mtime     : 更新日時     ctime参照"
		"\n"
		"\033[9G"
		"atime_cjd : アクセス日時 ctime_cjd参照"
		"\n"
		"\033[9G"
		"atime     : アクセス日時 ctime参照"
		"\n"
		"\033[9G"
		"*         : 全項目表示"
		"\n"
		...
		"\n"
		"\033[16G"
		"\"ctime like #{0m%}\" : ctime like '2010-12-%'"
		"\n"
		"\033[16G"

...
print_help()
{
	...
	P1(
		...
		IESC_STR1
		"\033[14G"	"abs(X)                        changes()                     char(X1,X2,...,XN)\n"
		"\033[14G"	"coalesce(X,Y,...)             concat(X,...)                 concat_ws(SEP,X,...)\n"
		"\033[14G"	"format(FORMAT,...)            glob(X,Y)                     hex(X)\n"
		"\033[14G"	"ifnull(X,Y)                   iif(X,Y,Z)                    instr(X,Y)\n"
		"\033[14G"	"last_insert_rowid()           length(X)                     like(X,Y)\n"
		"\033[14G"	"like(X,Y,Z)                   likelihood(X,Y)               likely(X)\n"
		"\033[14G"	"load_extension(X)             load_extension(X,Y)           lower(X)\n"
		"\033[14G"	"ltrim(X)                      ltrim(X,Y)                    max(X,Y,...)\n"
		"\033[14G"	"min(X,Y,...)                  nullif(X,Y)                   octet_length(X)\n"
		"\033[14G"	"printf(FORMAT,...)            quote(X)                      random()\n"
		"\033[14G"	"randomblob(N)                 replace(X,Y,Z)                round(X)\n"
		"\033[14G"	"round(X,Y)                    rtrim(X)                      rtrim(X,Y)\n"
		"\033[14G"	"sign(X)                       soundex(X)                    sqlite_compileoption_get(N)\n"
		"\033[14G"	"sqlite_compileoption_used(X)  sqlite_offset(X)              sqlite_source_id()\n"
		"\033[14G"	"sqlite_version()              substr(X,Y)                   substr(X,Y,Z)\n"
		"\033[14G"	"substring(X,Y)                substring(X,Y,Z)              total_changes()\n"
		"\033[14G"	"trim(X)                       trim(X,Y)                     typeof(X)\n"
		"\033[14G"	"unhex(X)                      unhex(X,Y)                    unicode(X)\n"
		"\033[14G"	"unlikely(X)                   upper(X)                      zeroblob(N)\n"
